(()=>{
  ...
  workflow.innerHTML='<div class="transfer-form"><h3>Prepare your direct transfer</h3><label>Choose the initiative<select id="support-initiative"><option value="FND">Willis Ouma Ondiek Foundation</option><option value="CMP">AWILO Campaign</option></select></label><label>Contribution amount (KES)<input id="donation-amount" type="number" min="1" step="1" inputmode="numeric" placeholder="Enter amount"></label><div class="amount-options" aria-label="Suggested amounts"><button type="button" data-amount="1000">KES 1,000</button><button type="button" data-amount="2500">KES 2,500</button><button type="button" data-amount="5000">KES 5,000</button><button type="button" data-amount="10000">KES 10,000</button></div><label>Programme or purpose<select id="donation-purpose"></select></label><label>Your name or organisation<input id="donor-name" autocomplete="name" required placeholder="Required for contribution records"></label><label class="source-declaration"><input id="lawful-source" type="checkbox"><span>I confirm that this contribution comes from a lawful source and the information supplied is accurate.</span></label><button class="page-button prepare-transfer" type="button">Generate transfer instructions</button></div><section class="transfer-summary" hidden aria-live="polite"><span class="launch-badge">TRANSFER INSTRUCTIONS READY</span><h3>Your direct KCB transfer</h3><dl><div><dt>Initiative</dt><dd id="summary-initiative"></dd></div><div><dt>Amount</dt><dd id="summary-amount"></dd></div><div><dt>Purpose</dt><dd id="summary-purpose"></dd></div><div><dt>Reference</dt><dd id="summary-reference"></dd></div></dl><div class="support-actions"><button class="copy-all" type="button">Copy all transfer details</button><a class="proof-link" target="_blank" rel="noopener">Send proof on WhatsApp</a><a class="receipt-link">Request acknowledgement by email</a></div><p class="transfer-privacy">Complete the transfer in your bank’s official app, website or branch. This website never asks for your banking password, PIN, card number or one-time code.</p></section>';
  ...
  const labels={FND:'Willis Ouma Ondiek Foundation',CMP:'AWILO Campaign'};
  ...
  workflow.querySelector('.prepare-transfer').onclick=()=>{
    if(!amount.value||Number(amount.value)<=0){amount.setCustomValidity('Enter a valid contribution amount.');amount.reportValidity();amount.setCustomValidity('');return}
    if(!name.value.trim()){name.setCustomValidity('Enter your name or organisation for contribution records.');name.reportValidity();name.setCustomValidity('');return}
    ...
    const details=labels[initiative.value]+' contribution\nAccount name: '+account.name+'\nBank: '+account.bank+'\nBranch: '+account.branch+'\nAccount number: '+account.number+'\nContributor: '+name.value.trim()+'\nAmount: '+formatted+'\nPurpose: '+purpose.value+'\nReference: '+reference;
    ...
    workflow.querySelector('.receipt-link').href='mailto:info@willisondiekfoundation.org?subject='+encodeURIComponent('Contribution acknowledgement — '+reference)+'&body='+encodeURIComponent(message+'\n\nPlease issue an acknowledgement or receipt.');
